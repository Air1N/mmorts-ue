    ...
    for (let i = 0; i < landPoints[k].length - 1; i += 1 + j) {
        ...
        for (let v = 0; v < j; v++) {
            landPoints[k].splice(i + 1, 0, {
                x: landPoints[k][i].x + ((landPoints[k][i + 1 + v].x - landPoints[k][i].x) / (j + 1)) * (v + 1) + (Math.random() * 2 - 1) * landSize / j, 
                y: landPoints[k][i].y + ((landPoints[k][i + 1 + v].y - landPoints[k][i].y) / (j + 1)) * (v + 1) + (Math.random() * 2 - 1) * landSize / j
            });
        }
    }
    }
