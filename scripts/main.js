...
function render() {
    ...
    for (let i = 0; i < landPoints.length; i++) {
        ...
        for (j = 1; j < landPoints[i].length - Math.max(Math.round(zoomLayers / zoomLevel), 1) - 1; j += Math.max(Math.round(zoomLayers / zoomLevel), 1)) {
            if (landPoints[i][j].x < 0 - 50 * zoomLevel || landPoints[i][j].x > display.width + 50 * zoomLevel) continue;
            if (landPoints[i][j].y < 0 - 50 * zoomLevel || landPoints[i][j].y > display.height + 50 * zoomLevel) continue;
